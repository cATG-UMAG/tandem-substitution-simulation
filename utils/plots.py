...
def tandem_distplot(family_name, show_pdf=True, split_axis=False):
    ...
    df_sim = pd.read_table(_find_filename("{}/{}/count_sim_size.tsv".format(TANDEM_SIM_SUMM_DIR, family_name)))
...
def cluster_size_distribution(family_name):
    ...
    df_real_bysize = df_real.groupby('size').agg({'seq_id': 'count'}).reset_index().rename(columns={'seq_id': 'real'})
    df_sim_bysize = df_sim.groupby('size').agg({'n': 'sum'}).reset_index().rename(columns={'n': 'simulation'})
    df_sim_bysize['simulation'] = np.round(df_sim_bysize['simulation'] / len(df_sim.simulation.unique()))  # get the average of all the simulations
    df_sim_bysize = df_sim_bysize[df_sim_bysize.simulation != 0]  # drop the rows with 0 tandems

    # join data in a single dataframe
    df_by_size = pd.merge(df_real_bysize, df_sim_bysize, how='outer').fillna(0)
    df_by_size = df_by_size.melt(id_vars='size', value_vars=['real', 'simulation'], var_name='target', value_name='n')
    ...
    for p in ax.patches:
        ax.annotate(int(p.get_height()), (p.get_x() + p.get_width() / 2., p.get_height()),
             ha='center', va='bottom', rotation=90, xytext=(0, 5), color='gray', textcoords='offset points')
    ax.set_ylim([0, max(df_by_size.n) * 1.2])
    ax.set(title="Distribution of mutation clusters by size ({})".format(family_name), xlabel="Cluster size", ylabel="Number of clusters")
    ax.legend()
    sns.despine()
...
def stop_codon_distribution(family_name):
    ...
    df_nostop = pd.read_table(_find_filename("{}/{}/count_sim_size.tsv".format(TANDEM_SIM_SUMM_DIR, family_name)))
    df_stop = pd.read_table(_find_filename("{}/{}/count_sim_size_stop.tsv".format(TANDEM_SIM_SUMM_DIR, family_name)))

    # group clusters by size
    df_nostop_bysize = df_nostop.groupby('size').agg({'n': 'sum'}).reset_index().rename(columns={'n': 'no stop codon'})
    df_nostop_bysize['no stop codon'] = np.round(df_nostop_bysize['no stop codon'] / len(df_nostop.simulation.unique()))
    df_nostop_bysize = df_nostop_bysize[df_nostop_bysize['no stop codon'] != 0]

    df_stop_bysize = df_stop.groupby('size').agg({'n': 'sum'}).reset_index().rename(columns={'n': 'stop codon'})
    df_stop_bysize['stop codon'] = np.round(df_stop_bysize['stop codon'] / len(df_stop.simulation.unique()))
    ...
    df_by_size = df_by_size.melt(id_vars='size', value_vars=['no stop codon', 'stop codon'], var_name='category', value_name='n')
    ...
    for p in ax.patches:
        ax.annotate(int(p.get_height()), (p.get_x() + p.get_width() / 2., p.get_height()),
             ha='center', va='bottom', rotation=90, xytext=(0, 5), color='gray', textcoords='offset points')
    ax.set_ylim([0, max(df_by_size.n) * 1.2])
    ax.set(title="Distribution of mutation clusters with stop codons ({})".format(family_name), xlabel="Cluster size", ylabel="Number of clusters")
    ax.legend()
    sns.despine()
...
def tandem_heatmap(family_name, target="real", stop_codons=False):
    ...
    if target == "real":
        df = pd.read_table(_find_filename("{}/{}.tsv".format(TANDEM_REAL_DIR, family_name)))
    elif target == 'simulated' and stop_codons is False:  # simulated
        df = pd.read_table(_find_filename("{}/{}/count_sim_mut.tsv".format(TANDEM_SIM_SUMM_DIR, family_name)))
    else:  # stop codons
        df = pd.read_table(_find_filename("{}/{}/count_sim_mut_stop.tsv".format(TANDEM_SIM_SUMM_DIR, family_name)))
    df = df[df['size'] == 2]
    ...
    with sns.axes_style("darkgrid"):
        ...
        ax.set(title="Tandem distribution {} from {} data ({})".format(stop_msg, target, family_name), xlabel="ALT", ylabel="REF")
...
def tandem_heatmap_percentage(family_name):
    ...
    df_sim = pd.read_table(_find_filename("{}/{}/count_sim_mut.tsv".format(TANDEM_SIM_SUMM_DIR, family_name)))
    df_sim = df_sim[df_sim['size'] == 2]
    ...
    # group by substitution and count
    grouped_real = df_real.groupby(['ref', 'alt']).agg({'seq_id': 'count'}).reset_index().rename(columns={'seq_id': 'n'})
    grouped_sim = df_sim.groupby(['ref', 'alt']).agg({'n': 'sum'}).reset_index()

    # add rows with 0 and make the tables
    table_real = pd.concat([grouped_real, dc_df]).drop_duplicates(subset=['ref', 'alt'], keep='first').pivot('ref', 'alt', 'n')
    table_sim = pd.concat([grouped_sim, dc_df]).drop_duplicates(subset=['ref', 'alt'], keep='first').pivot('ref', 'alt', 'n')
    ...
    with sns.axes_style("darkgrid"):
        fig, ax = plt.subplots(figsize=(13, 11))
        sns.heatmap(table.iloc[::-1], ax=ax, cmap="YlGnBu", annot=True, square=True, fmt='.0f', linewidths=1,
            cbar_kws={'label': "Percentage of false tandems (simulated/real)"})
        ax.set(title="Percentage of false Tandems ({})".format(family_name), xlabel="ALT", ylabel="REF")
...
def tandem_mutation_probability_heatmap(family_name, target="real", base2=True):
    ...
    if target == "real":
        ...
        df = pd.read_table(_find_filename("{}/{}/count_sim_pos.tsv".format(TANDEM_SIM_SUMM_DIR, family_name)))
        df = df[df['size'] == 2]
    ...
    df[['ref1', 'ref2']] = df['ref'].str.extractall('(.)')[0].unstack()  # split reference into 2

    # prepare 2 long-format dataframes with sequence position and reference base
    dfl_pos = pd.melt(df, id_vars=['id'], value_vars=['1', '2'], var_name='tposition', value_name='position')
    dfl_ref = pd.melt(df[['id', 'ref1', 'ref2']].rename(columns={'ref1': '1', 'ref2': '2'}),
                      id_vars=['id'], value_vars=['1', '2'], var_name='tposition', value_name='ref')

    # merge all
    dfl = pd.merge(pd.merge(dfl_pos, dfl_ref), df_prob[['position', 'mutation_probability']])

    # transform dataframe to put ref and mutation probability into separated columns for each nucleotide in tandem
    dfw = dfl.pivot(index='id', columns='tposition', values=['mutation_probability', 'ref']).reset_index()
    ...
    intervals = np.insert(np.logspace(-6, 0, 7, base=2), 0, 0) if base2 else np.arange(0, 1, 0.02)
    dfw_gr = dfw.groupby(['ref1', 'ref2', pd.cut(dfw['mp1'], intervals), pd.cut(dfw['mp2'], intervals)]).agg({'n': 'sum'}).reset_index()

    # combine mutation probability and ref in a single field to group later
    fmt_fn = _base2_str if base2 else lambda x: x
    dfw_gr['nucl1'] = dfw_gr.apply(lambda x: r"$({},{}]$ / {}".format(fmt_fn(x.mp1.left), fmt_fn(x.mp1.right), x.ref1), axis=1)
    dfw_gr['nucl2'] = dfw_gr.apply(lambda x: r"$({},{}]$ / {}".format(fmt_fn(x.mp2.left), fmt_fn(x.mp2.right), x.ref2), axis=1)

    # make the table
    t = dfw_gr.pivot(index='nucl1', columns='nucl2', values='n')
    # sort it correctly
    names_sorted = sorted(t.columns, key=lambda x: np.exp2(int(x[5:7])) if x[2] != '0' else 0)
    ...
    with sns.axes_style("darkgrid"):
        fig, ax = plt.subplots(figsize=(13, 11))
        sns.heatmap(t.iloc[::-1], ax=ax, cmap="YlGnBu", square=True, linewidths=1, cbar_kws={'label': "Number of tandems"})
        ax.set(title="Tandem distribution by mutation probability and reference of {} data ({})".format(target, family_name),
               xlabel="Mutation probability / Reference nucleotide nucleotide 2", ylabel="Mutation probability / Reference nucleotide nucleotide 1")
