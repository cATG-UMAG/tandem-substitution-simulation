...
def summarize(filename):
    ...
    for v in [False, True]:
        ...
        if len(subset) > 0:
            ...
            df_gr = pd.DataFrame(subset.groupby(["simulation", "size"]).size()).rename(
                columns={0: "n"}
            )
            df_gr.to_csv(
                path.join(
                    OUTPUT_DIR, method, name, "count_sim_size{}.tsv".format(suffix)
                ),
                sep="\t",
            )
            ...
            df_gr = pd.DataFrame(
                subset.groupby(["simulation", "ref", "alt", "size"]).size()
            ).rename(columns={0: "n"})
            df_gr.to_csv(
                path.join(
                    OUTPUT_DIR, method, name, "count_sim_mut{}.tsv".format(suffix)
                ),
                sep="\t",
            )
            ...
            df_gr = pd.DataFrame(
                subset.groupby(["simulation", "seq_id"]).size()
            ).rename(columns={0: "n"})
            df_gr.to_csv(
                path.join(
                    OUTPUT_DIR, method, name, "count_sim_seq{}.tsv".format(suffix)
                ),
                sep="\t",
            )
            ...
            df_gr = pd.DataFrame(
                subset.groupby(["simulation", "pos", "ref", "size"]).size()
            ).rename(columns={0: "n"})
            df_gr.to_csv(
                path.join(
                    OUTPUT_DIR, method, name, "count_sim_pos{}.tsv".format(suffix)
                ),
                sep="\t",
            )
